...
cv::Mat postprocess_outputs(const std::vector<std::vector<float>>& outputs, const cv::Mat& original_frame) {
    cv::Mat da_mask(224, 224, CV_8UC1);
    cv::Mat ll_mask(224, 224, CV_8UC1);
...
cv::Mat postprocess(float* da_output, float* ll_output, cv::Mat& original_frame) {
    ...
    // Criar as máscaras com shape (2, 224, 224)
...
int main() {
    ...
    while (true) {
        ...
        if (frameCount % 30 == 0) {
            ...
            double fps = std::chrono::duration<double>(now - start).count();
