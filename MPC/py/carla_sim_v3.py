...
import datetime
import math
from mpc_controller_v2 import process_mask
...
vehicle_model = None  # Inicialização global
...
def drive_waypoints(world, vehicle, waypoints, direction_forward=True, speed_factor=0.5):
    """Controla o veículo usando MPC para direção, parando se as linhas não forem detectadas."""
    global current_image, current_mask, frame_count, vehicle_model
    frame_count = 0
    
    if vehicle_model is None:
        from mpc_controller import VehicleModel
        vehicle_model = VehicleModel()  # Inicializar se ainda não foi
    ...
    while running and current_waypoint_index < total_waypoints:
        ...
        if not paused:
            ...
            # Inicializar controle com valores padrão
            vehicle_control.throttle = 0.0
            vehicle_control.steer = 0.0
            vehicle_control.brake = 1.0  # Freio ativado por padrão
            ...
            if current_image is not None and current_mask is not None:
                result = process_mask(current_mask)
                delta, y, psi, y_ref, lines_detected, x_center, line_params, predicted_path = result
                
                if lines_detected:
                    delta_max = math.pi / 6.0
                    steering = max(-1.0, min(1.0, delta / delta_max))
                    vehicle_control.steer = steering
                    vehicle_control.throttle = speed_factor  # Acelerar apenas se as linhas forem detectadas
                    vehicle_control.brake = 0.0  # Desativar freio
                    print(f"Step {frame_count}: y = {y:.9f} m, psi = {psi:.9f} rad, "
                          f"delta = {delta * 180.0 / math.pi:.9f} deg, y_ref = {y_ref:.9f} m")
                    frame_count += 1
                else:
                    print("Lines not detected, stopping vehicle.")
                    vehicle_control.throttle = 0.0
...
def main():
    ...
    try:
        ...
        world = client.load_world("/Game/Carla/Maps/Town04_Opt")
        ...
        camera_transform = carla.Transform(carla.Location(x=1.5, z=2.5), carla.Rotation(pitch=-20))
        ...
        if vehicle_model is None:
            from mpc_controller import VehicleModel
            vehicle_model = VehicleModel()  # Inicializar no main
