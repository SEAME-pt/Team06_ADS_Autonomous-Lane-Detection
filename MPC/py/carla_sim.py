...
import datetime
import math
from mpc_controller import process_mask  # Importar a função process_mask
...
def drive_waypoints(world, vehicle, waypoints, direction_forward=True, speed_factor=0.5):
    """Controla o veículo para seguir os waypoints, processando máscaras com MPC."""
    ...
    while running and current_waypoint_index < total_waypoints:
        ...
        if current_image is not None and current_mask is not None:
            ...
            # Processar a máscara com o MPC
            delta, y, psi, y_ref, lines_detected = process_mask(current_mask)
            
            # Exibir valores do MPC no terminal apenas se linhas forem detectadas
            if lines_detected:
                print(f"Step {frame_count}: y = {y:.9f} m, psi = {psi:.9f} rad, "
                      f"delta = {delta * 180.0 / math.pi:.9f} deg, y_ref = {y_ref:.9f} m")
                frame_count += 1
            
            # Adicionar informações à imagem exibida
            ...
            cv2.putText(img_display, f"MPC Delta: {delta * 180.0 / math.pi:.2f} deg", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            cv2.putText(img_display, f"Lines Detected: {lines_detected}", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            ...
            # Exibir a máscara na nova janela com ROI destacada
...
def process_segmentation(image):
    """Processa imagens de segmentação para criar uma máscara binária para faixas de estrada (ID 24)."""
    global current_mask
    image.convert(carla.ColorConverter.Raw)
    arr = np.frombuffer(image.raw_data, dtype=np.uint8).reshape((image.height, image.width, 4))
    class_ids = arr[:, :, 2]
    unique_classes = np.unique(class_ids)
    #print(f"IDs de classes na imagem: {unique_classes}")
    
    # Criar máscara binária para ID 24
    mask = (class_ids == 24).astype(np.uint8) * 255
    
    # Aplicar fechamento morfológico para conectar linhas tracejadas
    kernel = np.ones((7, 7), np.uint8)  # Kernel 5x5
...
def main():
    ...
    try:
        ...
        cv2.namedWindow("CARLA Mask", cv2.WINDOW_NORMAL)  # Nova janela para a máscara
