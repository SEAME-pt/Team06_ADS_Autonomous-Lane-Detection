...
// Pós-processa saídas do modelo: Gera máscaras e frame com linhas
std::pair<cv::Mat, cv::Mat> postprocess(float* da_output, float* ll_output, cv::Mat& original_frame, std::vector<cv::Point>& medianPoints) {
    ...
    // Processar máscara da área transitável para obter bordas e mediana
    MaskProcessor processor;
    cv::Mat mask_output;
    processor.processMask(da_resized, mask_output, medianPoints);

    // Criar frame com máscaras de área transitável e linhas
    ...
    for (int y = 0; y < result_frame.rows; ++y) {
        for (int x = 0; x < result_frame.cols; ++x) {
            if (da_resized.at<uchar>(y, x) > 100) {
                result_frame.at<cv::Vec3b>(y, x) = cv::Vec3b(255, 0, 0); // Azul para área transitável
            }
            if (ll_resized.at<uchar>(y, x) > 100) {
                result_frame.at<cv::Vec3b>(y, x) = cv::Vec3b(0, 255, 0); // Verde para linhas
            }
        }
    }

    // Desenhar linhas das bordas e mediana no frame original
    cv::Mat result_frame_with_lines = original_frame.clone();
    // Reutilizar os pontos da mediana e calcular bordas novamente para consistência
    if (!medianPoints.empty()) {
        ...
        for (int y = 0; y < mask_bin.rows; y++) {
            ...
            // Acessar métodos públicos ou replicar lógica para evitar acesso a métodos privados
        ...
        if (left_coeffs.valid && right_coeffs.valid) {
            ...
            for (int y = top_y; y <= bottom_y; y++) {
                ...
                if (left_x >= 0 && left_x < result_frame_with_lines.cols)
                    left_line_points.push_back(cv::Point(left_x, y));
                if (right_x >= 0 && right_x < result_frame_with_lines.cols)
            ...
            if (!left_line_points.empty() && !right_line_points.empty()) {
                cv::line(result_frame_with_lines, left_line_points.front(), left_line_points.back(), cv::Scalar(0, 0, 255), 2); // Vermelho
                cv::line(result_frame_with_lines, right_line_points.front(), right_line_points.back(), cv::Scalar(255, 0, 0), 2); // Azul
            }

            // Desenhar mediana no frame original
            if (!medianPoints.empty()) {
                cv::line(result_frame_with_lines, medianPoints.front(), medianPoints.back(), cv::Scalar(0, 255, 0), 2); // Verde
            }
        }
    }

    // Retornar ambas as imagens: máscara processada e frame com linhas
    return std::make_pair(mask_output, result_frame_with_lines);
}