...
L = 2.9  # Comprimento do veículo (em metros, manter como está)
...
# Parâmetros do NMPC
N = 12  # Horizonte de previsão
Q = np.diag([10.0, 10.0, 20.0, 20.0])  # Pesos para seguir a trajetória
R = np.diag([10.0, 0.5])  # Penalidade nos controles

# Escala aproximada: assumindo que 1 metro ≈ 100 pixels (ajustar conforme necessário)
...
def prepare_reference(center_line):
    ...
    # Escalar os pontos para metros (assumindo 1 metro ≈ 100 pixels)
    scaled_points = points / PIXELS_PER_METER

    # Combinar em (x, y, psi, v)
    reference = np.column_stack((scaled_points, psi_ref, v_ref))
    return reference, psi_ref
...
def main(center_line):
    ...
    reference, psi_ref = prepare_reference(center_line)
    if len(reference) == 0:
    ...
    # Estado inicial do veículo alinhado com a primeira posição da linha central (escalado para metros)
    if len(center_line) > 0:
        state_init = np.array([center_line[0][0] / PIXELS_PER_METER, center_line[0][1] / PIXELS_PER_METER, psi_ref[0], 5.0])  # (x, y, psi, v)
    else:
        state_init = np.array([0.0, 0.0, 0.0, 5.0])  # Fallback
    ...
    # Re-escalar as previsões para pixels antes de retornar
    X_pred_scaled = X_pred * PIXELS_PER_METER
    return X_pred_scaled, state_init
