...
def process_mask(mask_path, nmpc_pred=None, current_state=None):
    ...
    if nmpc_pred is not None and current_state is not None:
        ...
        x_current, y_current = current_state[0] * 100.0, current_state[1] * 100.0  # Re-escalar para pixels

        # Imprimir valores para depuração
        print("Predicted trajectory points (x_pred, y_pred) in pixels:")
        for i in range(len(x_pred)):
            x_pixel = x_pred[i] * 100.0  # Re-escalar para pixels
            y_pixel = y_pred[i] * 100.0
            print(f"  Step {i}: ({x_pixel:.1f}, {y_pixel:.1f})")
        print(f"Current position in pixels: x={x_current:.1f}, y={y_current:.1f}")

        # Desenhar a trajetória predita como uma linha tracejada (vermelha)
        for i in range(len(x_pred) - 1):
            x1 = int(x_pred[i] * 100.0)  # Re-escalar para pixels
            y1 = int(y_pred[i] * 100.0)
            x2 = int(x_pred[i + 1] * 100.0)
            y2 = int(y_pred[i + 1] * 100.0)
