...
: L_(L), dt_(dt), Np_(Np), Nc_(Nc), delta_max_(delta_max), a_max_(a_max) {
    setup_nmpc();
}
...
void NMPCController::setup_nmpc() {
    ...
    X_ = opti_.variable(4, Np_ + 1); // Estados: [x, y, psi, v] para Np+1 passos
    U_ = opti_.variable(2, Nc_);     // Controles: [delta, a] para Nc passos
    
    // Matrizes de peso
    casadi::DM Q = casadi::DM::diag({1.0, 1.0, 1.0, 0.1}); // Peso para estados
    casadi::DM R = casadi::DM::diag({0.1, 0.1});           // Peso para controles
    casadi::DM P = Q;                                      // Peso terminal
    
    // Função de custo
    casadi::MX cost = 0;
    x_ref_params_.resize(Np_);
    for (int k = 0; k < Np_; ++k) {
        x_ref_params_[k] = opti_.parameter(4, 1); // Parâmetro para cada x_ref_k
    ...
    // Restrições
    x0_param_ = opti_.parameter(4, 1); // Estado inicial
